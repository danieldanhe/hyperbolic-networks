...
\begin{frame}{Heterogeneity and clustering}
  \begin{tabular}{lll}
    ...
    Low clustering & \makecell{\input{figures/generate_points/low_clust_low_hetero.tikz}} & \makecell{\input{figures/generate_points/low_clust_high_hetero.tikz}} \\
    High clustering & \makecell{\input{figures/generate_points/high_clust_low_hetero.tikz}} & \makecell{\input{figures/generate_points/high_clust_high_hetero.tikz}} \\ \bottomrule
  \end{tabular}
...
\begin{frame}{The Poincaré disc}
  ...
  \onslide<4->{
    \begin{example}
      \(\{3,7\}\)\quad
      \includegraphics[height=2cm]{figures/tilings/tiling-3-7.pdf}
      \qquad
      \(\{4,5\}\)\quad
      \includegraphics[height=2cm]{figures/tilings/tiling-4-5.pdf}
      \qquad
      \(\{5,4\}\)\quad
      \includegraphics[height=2cm]{figures/tilings/tiling-5-4.pdf}
    \end{example}
  }
\end{frame}
...
\begin{frame}{Network generation in hyperbolic space}
  \onslide<1->{There are \(N\) nodes. Each node \(i\) is assigned coordinates \((r_i,\theta_i)\) on the hyperbolic plane.}
  \begin{itemize}
    ...
    where
    \[\alpha=\frac{\gamma-1}2,\quad L\approx2\ln\biggl(\frac{2N}{\pi\bar k}\Bigl(\frac{\gamma-1}{\gamma-2}\Bigr)^2\biggr)\]
    }
...
\begin{frame}{Network generation in hyperbolic space}
  \onslide<1->{For any two nodes \(i,j\), find the hyperbolic distance
  \begin{align*}
    x_{i,j}&=\arcosh\Bigl(\cosh r_i\cosh r_j-\sinh r_i\sinh r_j\cos\bigl(\min(\lvert\theta_i-\theta_j\rvert,2\pi-\lvert\theta_i-\theta_j\rvert)\bigr)\Bigr) \\
    &\approx r_i+r_j+2\ln\Bigl(\frac{\min(\lvert\theta_i-\theta_j\rvert,2\pi-\lvert\theta_i-\theta_j\rvert)}2\Bigr)
  \end{align*}}
  \onslide<2->{Connect with connection probability
  \[p(x_{i,j})=\frac1{e^{\beta(x_{i,j}-L)/2}+1}\]
  }
  \onslide<3->{Clustering is maximized when \(\beta\to\infty\); the probability reduces to a step function (connect only if \(x_{i,j}<L\)).} \onslide<4->{The inverse of \(\beta\) is called the \imp{temperature \(T\).}}

  \onslide<5->{We operate under the \imp{cold regime} – that \(\beta>1\).}
\end{frame}
...
\begin{frame}{Visualizing the model}
  ...
  \begin{tabular}{llllll}
    ...
    \(\gamma=2.1\) & \makecell{\input{figures/poincare_model/network_gamma_2.1_20.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_2.1_50.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_2.1_100.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_2.1_200.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_2.1_500.tikz}} \\
    \(\gamma=2.5\) & \makecell{\input{figures/poincare_model/network_gamma_2.5_20.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_2.5_50.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_2.5_100.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_2.5_200.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_2.5_500.tikz}} \\
    \(\gamma=3.0\) & \makecell{\input{figures/poincare_model/network_gamma_3.0_20.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_3.0_50.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_3.0_100.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_3.0_200.tikz}} & \makecell{\input{figures/poincare_model/network_gamma_3.0_500.tikz}} \\ \bottomrule
  \end{tabular}
\end{frame}
