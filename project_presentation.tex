...
\begin{frame}[standout]
  ...
  \begin{multicols*}{2}
    ...
    \qrcode[hyperlink,height=3cm,version=4]{https://github.com/danieldanhe/hyperbolic-networks}

    \medskip

    Code and presentation \par
    {\mdseries\url{https://github.com/danieldanhe/hyperbolic-networks}}

    \columnbreak

    \qrcode[hyperlink,height=3cm,version=4]{https://hyperbolic-networks.vercel.app}

    \medskip

    Live web app \par
    {\mdseries\url{https://hyperbolic-networks.vercel.app}}
  \end{multicols*}
\end{frame}
...
\begin{frame}{Network generation in hyperbolic space: Einsteinian model}
  ...
  Clustering is maximized when \(\beta\to\infty\); the probability reduces to a step function (connect only if \(x_{i,j}<R\)). \pause The inverse of \(\beta\) is called the \imp{temperature \(T\).} \pause
...
\begin{frame}{Why does routing matter?}
  \begin{example}
    \begin{itemize}
      \parskip=\dimexpr\parskip/2\relax
      \item Social network navigation (“six degrees of separation”): You can get a letter to a stranger by relying on only friends within six moves. But how? \pause
      \item Internet packet routing: Your laptop sends data to a server in Japan. How do routers decide the path? \pause
      \item Neural signal routing: Your finger touches something hot. How does the signal reach the right motor neurons to pull away? \pause
      \item Supply chain logistics: An important hub is down. How can you transport a product to a customer? \pause
      \item Epidemic spreading control: A city experiences an outbreak of an infectious disease. Which cities should close borders to prevent spread? \pause
      \item Music recommendations: A friend and you want a shared playlist. How does the app find artists that bridge your tastes?
    \end{itemize}
  \end{example}
\end{frame}

\begin{frame}{Why do we need a good algorithm?}
  Routing requires efficient paths. But in the real world, there is no way to look at networks as a whole and find the most efficient path. \pause

  How do nodes find efficient paths quickly using only
  \begin{itemize}[<+->]
    \item their own coordinates
    \item their neighbours’ coordinates
    \item the destination’s coordinates
  \end{itemize}
\end{frame}
...
\begin{frame}{Bidirectional routing}
  ...
  Starting routing from two nodes simultaneously and expanding both searches until they meet is surprisingly fast for hyperbolic networks, amounting to sublinear time. When \(\gamma=2.5\), bidirectional routing is \(\mathcal O(N^{2/3})\).\footcite{Bläsius_2018}
\end{frame}
...
\begin{frame}{Equivalence of the models}
  ...
  \[\chi_{i,j}=e^{(x_{i,j}-R)/2}\] \pause

  But the Newtonian model is easier to optimize. \(\varkappa\) has an analytical solution, and \(\vartheta\) optimizes numerically and independently. \pause That’s why we’ll embed using the Newtonian model and then convert to the Einsteinian model for routing.
\end{frame}
...
\begin{frame}{Layered approach}
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \[\vartheta_i=\arctan\Bigl(\frac{\sum_j\sin\vartheta_j}{\sum_j\cos\vartheta_j}\Bigr)\quad\text{for connected nodes \(j\)}\] \pause
    \end{itemize}
  \end{itemize}
